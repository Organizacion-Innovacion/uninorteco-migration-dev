...
} from "../src/domain-logic/semester-algorithms";
import { Course } from "../src/entities/course";
import { createTestAcademicSemester } from "./test-utils";

function replaceGradeOfUnLockedCourses(courses: Course[], grade: number) {
  ...
  describe("computeFinalGradeOfSemester", () => {
    test.each([
      ...
      (grades, weights, expected) => {
        ...
        const result = computeFinalGradeOfSemester(academicSemester.courses);
  ...
  describe("computeNeededGradeForSemester", () => {
    test.each([
      ...
      (desiredGrade, grades, credits, isLocked, expected) => {
        ...
        replaceGradeOfUnLockedCourses(semester.courses, neededGrade);
        const finalGrade = computeFinalGradeOfSemester(semester.courses);
    ...
    it("should ignore courses with 0 credits", () => {
      ...
      const semester = createTestAcademicSemester(grades, credits, isLocked);
      const neededGrade = computeNeededGradeForSemester(semester.courses, desiredGrade);
    ...
    it("should throw an error if all components are locked", () => {
      ...
      const semester = createTestAcademicSemester(grades, credits, isLocked);
      try {
        computeNeededGradeForSemester(semester.courses, desiredGrade);
      } catch (error) {
